...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            } else if ("javaSearchModelGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseJavaSearchModelGenerator(context, childNode);
    ...
    protected void parseJavaSearchModelGenerator(Context context, Node node) {
        JavaSearchModelGeneratorConfiguration javaSearchModelGeneratorConfiguration = new JavaSearchModelGeneratorConfiguration();

        context.setJavaSearchModelGeneratorConfiguration(javaSearchModelGeneratorConfiguration);

        Properties attributes = parseAttributes(node);
        String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$

        javaSearchModelGeneratorConfiguration.setTargetPackage(targetPackage);
        javaSearchModelGeneratorConfiguration.setTargetProject(targetProject);
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseProperty(javaSearchModelGeneratorConfiguration, childNode);
            }
        }
    }
